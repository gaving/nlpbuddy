...
fasttext_path = "/usr/local/bin/fasttext"
...
MODEL_MAPPING = {"en": "/app/models/ag_news.bin"}
...
POS_MAPPING = {"NOUN": "nouns", "VERB": "verbs", "ADJ": "adjectives"}
...
def analyze_text(text):
    ...
    lexical_attrs = {"urls": [], "emails": [], "nums": []}
    for token in doc:
        if token.like_url:
    ...
    for token in doc:
        ...
        if lemmatized_token in indexes:
            ...
            if pos_flag:
                ...
                subjectivity_score += subj_scores[
                    df.at[indx, "Subjectivity" + str(match_col_index)]
                ]
                anger_score += emotion_scores[
                    str(df.at[indx, "Anger" + str(match_col_index)])
                ]
                disgust_score += emotion_scores[
                    str(df.at[indx, "Disgust" + str(match_col_index)])
                ]
                fear_score += emotion_scores[
                    str(df.at[indx, "Fear" + str(match_col_index)])
                ]
                happiness_score += emotion_scores[
                    str(df.at[indx, "Happiness" + str(match_col_index)])
                ]
                sadness_score += emotion_scores[
                    str(df.at[indx, "Sadness" + str(match_col_index)])
                ]
                surprise_score += emotion_scores[
                    str(df.at[indx, "Surprise" + str(match_col_index)])
                ]
                matched_tokens += 1
    try:
